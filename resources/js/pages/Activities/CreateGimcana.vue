...
const form = ref({
  ...
  type: 'gimcana',
  locations: [
    { name: '', description: '', statement: '', question_type: 'open', answers: [''], correct_answer: '', latitude: '', longitude: '' }
  ]
});

// add new location
const addLocation = () => {
  form.value.locations.push({ name: '', description: '', statement: '', question_type: 'open', answers: [''], correct_answer: '', latitude: '', longitude: '' });
};
const removeLocation = (index: number) => {
  form.value.locations.splice(index, 1);
};

const setLocationCoords = (index: number, coords: { latitude: number, longitude: number }) => {
    ...
    <div class="overflow-hidden">
      <div class="relative container pt-10 pb-10">
        <div class="mb-8 flex flex-col gap-3">
          <h1 class="text-3xl font-semibold text-hp-text">Crea gimcana</h1>
        </div>
        ...
        <form @submit.prevent="createGimcana" class="grid gap-8 lg:grid-cols-3">
          <section class="space-y-6 lg:col-span-2">
            <div class="rounded-3xl border border-hp-border p-6 shadow-sm">
              <h2 class="text-lg font-semibold text-hp-text">Dades principals</h2>
            ...
            <div class="rounded-3xl border border-hp-border bg-hp-bg-card p-6 shadow-sm">
              <div class="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <h2 class="text-lg font-semibold text-hp-text">Parades</h2>
                </div>
                <button type="button" @click="addLocation"
                  class="inline-flex items-center gap-2 rounded-full bg-hp-primary px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-hp-primary-light hover:border-2 border-2 border-hp-primary-light hover:border-hp-primary">
                ...
                <div v-for="(location, index) in form.locations" :key="index"
                  ...
                  <div class="flex flex-wrap items-center justify-between gap-3">
                    <div class="flex items-center gap-3">
                      <span class="flex h-9 w-9 items-center justify-center rounded-full bg-hp-primary text-sm font-semibold text-white">
                        {{ index + 1 }}
                      </span>
                      <div>
                        <p class="text-hp-text">Parada {{ index + 1 }}</p>
                      </div>
                    </div>
                    <button type="button" @click="removeLocation(index)"
                      class="text-xs font-semibold uppercase tracking-wider text-red-500 hover:text-red-600">
                      Eliminar
                    </button>
                  </div>
                  ...
                  <div class="mt-4 grid gap-4 lg:grid-cols-4">
                    <div class="space-y-4 lg:col-span-2">
                      ...
                      <div v-if="location.question_type === 'multiple_choice'" class="space-y-3">
                        <div class="flex items-center justify-between">
                          <label>Respostes</label>
                          <button type="button" @click="addAnswer(index)"
                            class="text-xs font-semibold uppercase tracking-wider text-emerald-600 hover:text-emerald-500">
                            Afegir resposta
                          </button>
                        </div>
                        <div class="space-y-2">
                          <div v-for="(answer, answerIndex) in location.answers" :key="answerIndex"
                            class="flex items-center gap-2">
                            <input v-model="location.answers[answerIndex]" type="text" placeholder="Resposta"
                              class="w-full rounded-xl border border-hp-border bg-hp-bg-card p-2 shadow-sm focus:border-hp-primary focus:outline-none focus:ring-2 focus:ring-hp-primary/50"
                              required />
                            <button type="button" @click="removeAnswer(index, answerIndex)"
                              class="text-xs font-semibold uppercase tracking-wider text-red-500 hover:text-red-600">
                              Eliminar
                            </button>
                          </div>
                        ...
                        <div>
                          ...
                          <select v-model="location.correct_answer"
                            class="mt-2 w-full rounded-xl border border-hp-border bg-hp-bg-card p-2 text-sm shadow-sm focus:border-hp-primary focus:outline-none focus:ring-2 focus:ring-hp-primary/50"
                            required>
                            <option value="">Selecciona</option>
                            <option v-for="(answer, answerIndex) in location.answers" :key="answerIndex" :value="answer">
                    ...
                    <div class="rounded-2xl border border-hp-border bg-hp-bg-card p-3 shadow-sm lg:col-span-2">
                      <h3 class="text-sm font-semibold text-slate-700">Mapa del punt</h3>
          ...
          <aside class="space-y-5">
            <div class="rounded-3xl border border-hp-border bg-hp-bg-card p-5 shadow-sm lg:sticky lg:top-6">
              <div class="flex items-center justify-between">
                <h2 class="text-base font-semibold text-hp-text">Mapa general</h2>
              ...
              <div class="mt-4 space-y-2 text-xs text-slate-600">
                <div v-for="(marker, index) in overviewMarkers" :key="`${marker.name}-${index}`"
                  ...
                  <span class="font-medium text-slate-700">{{ marker.name }}</span>
                  <span class="text-slate-500">{{ marker.latitude }}, {{ marker.longitude }}</span>
                </div>
              </div>
            </div>
          </aside>
