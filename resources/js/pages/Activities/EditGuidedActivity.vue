<script setup lang="ts">
import { Form, Link } from '@inertiajs/vue3';
import { ref } from 'vue';
import { update } from '@/routes/exchange/guidedactivity';

interface Activity {
    id: number;
    title: string;
    description?: string;
    start_date?: string;
    end_date?: string;
    exchange_id: number;
}

const props = defineProps<{
    activity: Activity;
}>();

const preview = ref<string | null>(null);

const handleFileChange = (event: Event) => {
    const input = event.target as HTMLInputElement;

    if (input.files && input.files[0]) {
        preview.value = URL.createObjectURL(input.files[0]);
    }
};
</script>

<template>
    <div class="flex min-h-screen items-center justify-center bg-gray-100 p-4 dark:bg-gray-900">
        <div class="w-full max-w-md">

            <!-- Header -->
            <div class="mb-6 text-center">
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">
                    Edita l'activitat
                </h1>
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                    Modifica la informació de l'activitat
                </p>
            </div>

            <!-- Card -->
            <div class="rounded-2xl bg-white p-4 shadow-lg dark:bg-gray-800">

                <Form
                    :action="update({
                        exchange: activity.exchange_id,
                        guidedactivity: activity.id
                    })"
                    method="put"
                    class="space-y-5"
                >

                    <!-- Títol -->
                    <div>
                        <label class="text-sm font-medium text-gray-700 dark:text-gray-300">
                            Títol
                        </label>
                        <input
                            type="text"
                            name="title"
                            v-model="activity.title"
                            class="mt-1 w-full rounded-md border px-3 py-2 text-sm focus:ring-2 focus:ring-teal-400 focus:outline-none dark:border-gray-700 dark:bg-gray-900 dark:text-white"
                        />
                    </div>

                    <!-- Descripció -->
                    <div>
                        <label class="text-sm font-medium text-gray-700 dark:text-gray-300">
                            Descripció
                        </label>
                        <textarea
                            name="description"
                            rows="3"
                            v-model="activity.description"
                            class="mt-1 w-full rounded-md border bg-gray-50 px-3 py-2 text-sm focus:ring-2 focus:ring-teal-400 focus:outline-none dark:border-gray-700 dark:bg-gray-900 dark:text-white"
                        />
                    </div>

                    <!-- Dates -->
                    <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                        <div>
                            <label class="text-sm font-medium text-gray-700 dark:text-gray-300">
                                Comença
                            </label>
                            <input
                                type="datetime-local"
                                name="start_date"
                                v-model="activity.start_date"
                                class="mt-1 w-full rounded-md border border-gray-200 bg-gray-50 px-3 py-2 text-sm focus:ring-2 focus:ring-teal-400 focus:outline-none dark:border-gray-700 dark:bg-gray-900"
                            />
                        </div>

                        <div>
                            <label class="text-sm font-medium text-gray-700 dark:text-gray-300">
                                Acaba
                            </label>
                            <input
                                type="datetime-local"
                                name="end_date"
                                v-model="activity.end_date"
                                class="mt-1 w-full rounded-md border border-gray-200 bg-gray-50 px-3 py-2 text-sm focus:ring-2 focus:ring-teal-400 focus:outline-none dark:border-gray-700 dark:bg-gray-900"
                            />
                        </div>
                    </div>

                    <!-- Imatge -->
                    <div>
                        <label class="mb-3 block text-sm font-medium text-gray-900 dark:text-gray-300">
                            Imatge
                        </label>

                        <label
                            class="flex h-40 w-full cursor-pointer flex-col items-center justify-center rounded-2xl border-2 border-dashed border-teal-300 transition hover:border-teal-500 hover:bg-teal-50"
                        >
                            <span class="text-sm text-gray-600">
                                Fes clic per canviar la imatge
                            </span>

                            <input
                                type="file"
                                name="file"
                                class="hidden"
                                @change="handleFileChange"
                            />
                        </label>

                        <!-- Preview -->
                        <div v-if="preview" class="mt-4">
                            <img
                                :src="preview"
                                class="h-40 w-full rounded-xl object-contain"
                            />
                        </div>
                    </div>

                    <!-- Botons -->
                    <div class="flex w-full gap-3 pt-2">
                        <Link
                            href="/activities"
                            class="flex-1 rounded-md border border-gray-300 py-2 text-center text-sm dark:border-gray-600"
                        >
                            Cancel·lar
                        </Link>

                        <button
                            type="submit"
                            class="flex-1 rounded-md bg-teal-400 py-2 text-sm font-semibold hover:bg-teal-300"
                        >
                            Guardar canvis
                        </button>
                    </div>

                </Form>
            </div>
        </div>
    </div>
</template>