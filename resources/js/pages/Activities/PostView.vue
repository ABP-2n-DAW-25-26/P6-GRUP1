    ...
    <div class="p-4 sm:p-6">
        <div class="mx-auto w-full max-w-4xl">
            <div class="rounded-2xl sm:p-6">
                <p
                    class="mb-2 font-hp text-lg font-semibold tracking-wide text-hp-primary uppercase"
                >
                    Anunci
                </p>
                <h1
                    class="mb-3 text-4xl font-bold text-hp-primary-dark lg:text-7xl"
                >
                    {{ post.title }}
                </h1>

                <div
                    class="mb-5 flex flex-row justify-between gap-20 rounded-xl bg-hp-bg/30 p-4 text-sm text-hp-text-dim sm:grid-cols-2"
                >
                    <div class="rounded border-l-3 border-hp-primary pl-3">
                        <p>Inici</p>
                        <p>
                            <strong>{{ formatDate(post.start_date) }}</strong>
                        </p>
                        <p>{{ formatTime(post.start_date) }}</p>
                    </div>
                    <div
                        class="rounded border-r-3 border-hp-primary pr-3 text-right"
                    >
                        <p>Fi</p>
                        <p>
                            <strong>{{ formatDate(post.end_date) }}</strong>
                        </p>
